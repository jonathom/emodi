...
df <- data.frame(method=NA, sample=NA, iteration=NA, RMSE=NA, exhaustive=NA)
for (method in list.files("./CVresults")) {
  ...
  for (smpl in samples) {
    for (iteration in 1:length(out_files)) {
      ...
      df[iteration,] <- c(method, smpl, iteration, RMSE, NA)
      ...
      gd <- CAST::plot_geodist(x = AGBdata, modeldomain = mask, stat='ecdf') 
    }
  }
}
```
